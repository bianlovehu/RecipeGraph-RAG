        ...
        try:
            if analysis.recommended_strategy == SearchStrategy.HYBRID_TRADITIONAL:
                ...
                if not documents:
                    ...
                    analysis.fallback_reason = "graph_rag_empty"
                    documents = self.traditional_retrieval.hybrid_search(query, top_k)
                
            elif analysis.recommended_strategy == SearchStrategy.COMBINED:
                logger.info("🔄 使用组合检索策略")
                documents, graph_available = self._combined_search(query, top_k)
            ...
            analysis.fallback_used = True
            analysis.fallback_reason = f"route_error:{type(e).__name__}"
            documents = self.traditional_retrieval.hybrid_search(query, top_k)
            return documents, analysis
    
    def _combined_search(self, query: str, top_k: int) -> Tuple[List[Document], bool]:
        """
        组合搜索策略：结合传统检索和图RAG的优势
        """
        candidate_k = top_k * getattr(self.config, "retrieval_candidate_multiplier", 3)
        channels = self.traditional_retrieval.retrieve_channels(query, top_k)
        graph_docs = self.graph_rag_retrieval.graph_rag_search(query, candidate_k)
        if graph_docs:
            channels["graph"] = graph_docs
        ...
        return documents, bool(graph_docs)
