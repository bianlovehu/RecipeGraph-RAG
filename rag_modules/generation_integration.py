...
class GenerationIntegrationModule:
    ...
    def _create_text_completion(self, prompt: str, max_attempts: int = 2) -> str:
        """生成非思考模式文本，并拒绝空响应或被截断的响应。"""
        last_error = None
        for attempt in range(max(1, max_attempts)):
            try:
                ...
                if getattr(choice, "finish_reason", None) == "length":
                    raise ValueError("模型回答因 max_tokens 被截断")
        ...
        raise RuntimeError(f"答案生成连续失败 {max(1, max_attempts)} 次: {last_error}")
    ...
    def generate_adaptive_answer(self, question: str, documents: List[Document]) -> str:
        ...
        return self._create_text_completion(prompt)
